defmodule ChaskiWeb.Api.V1.ClientApplicationController do
  ...
  def create(conn, %{"client_application" => client_application_params}) do
    with {:ok, %ClientApplication{} = client_application} <- ClientApplications.create_client_application(client_application_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", Routes.api_v1_client_application_path(conn, :show, client_application))
  ...
  def update(conn, %{"id" => id, "client_application" => client_application_params}) do
    ...
    with {:ok, %ClientApplication{} = client_application} <- ClientApplications.update_client_application(client_application, client_application_params) do
      render(conn, "show.json", client_application: client_application)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %ClientApplication{}} <- ClientApplications.delete_client_application(client_application) do
